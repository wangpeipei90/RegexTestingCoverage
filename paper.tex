...
\begin{abstract}
%Regular expressions are used for querying databases, grepping file content, document searches, validating user input, and parsing files. 
%Most programming languages have standard libraries or built-in support for regular expression processing. 
%Despite their frequent appearance in source code, regular expressions are responsible for numerous faults in many software products, cause software performance issues, and often are hard to understand and maintain. Further, d
...
%These metrics allow us to identify how much of a regular expression's structure is covered by existing tests. 
Using over 15,000 tested regular expressions in 1,225 Java projects on GitHub, we measure node, edge, and edge-pair coverage. 
...
%Next, we compare the coverage achieved in the test suites to coverage that could be achieved using Rex, an off-the-shelf . 
...
%We found that for the same number of test inputs,  Rex achieves lower coverage than the existing tests because Rex does not generate non-matching input strings. 
%This indicates that tools such as Rex can be used to write test inputs with similar coverage to the developer tests
\end{abstract}
